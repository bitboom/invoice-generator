...
describe('shared logo, theme, and template behavior', () => {
  ...
  it('renders high-resolution export PNGs and can split a long classic invoice into two files', () => {
    ...
    assert.match(app, /const longShareRef = useRef\(null\);/);
    assert.match(app, /className="export-frame export-long-frame"/);
    assert.match(app, /longShare=\{true\}/);
    assert.match(app, /const shareNode = exports\.length > 1 \? longShareRef\.current\?\.querySelector\('\.invoice'\) : nodes\[0\];/);
    assert.match(app, /filename: exports\.length > 1 \? currentCombinedFilename\(\) : exports\[0\]\.filename/);
    assert.match(app, /text: exports\.length > 1 \? `인보이스 PNG \$\{exports\.length\}페이지 긴 견적서` : '인보이스 PNG'/);
    assert.doesNotMatch(app, /combinePageCanvases/);
    assert.match(templates, /longShare = false/);
    assert.match(templates, /tpl-classic-long-share/);
    assert.match(index, /\.export-long-frame/);
    assert.match(index, /\.tpl-classic\.tpl-classic-long-share/);
  ...
  it('lets users reorder line items in the editor without collapsing the item-name field', () => {
    ...
    assert.match(app, /className="item-summary"/);
    assert.match(app, /<span>합계<\/span><strong>₩\{fmt\(totals\.supply\)\}<\/strong>/);
    ...
    assert.doesNotMatch(app, /const lineItemAmountValue = \(it\) =>/);
    assert.doesNotMatch(app, /const setItemAmount = \(idx, value\) =>/);
    assert.doesNotMatch(app, /총액 ₩\{fmt\(totals\.total\)\}/);
    assert.match(app, /aria-label="위로 이동"/);
    assert.match(app, /aria-label="아래로 이동"/);
    assert.match(index, /\.item-summary/);
    assert.match(index, /\.item-summary strong/);
